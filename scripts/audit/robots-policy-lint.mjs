...
// of truth in scripts/audit/bot-policy.md. HARD gate on the core rule:
...
export function loadPolicy() {
  const md = read(join(REPO_ROOT, "scripts", "audit", "bot-policy.md"));
...
export function run(ctx) {
  ...
  const findings = [];
  let policy;
  try { policy = loadPolicy(); } catch (e) { return result("robots-policy", sev, [{ level: "error", msg: e.message, file: "scripts/audit/bot-policy.md" }]); }
