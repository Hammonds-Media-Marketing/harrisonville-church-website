...
//   · manual runs (npm run audit)                 (default)
//   · Next.js prebuild and CI                     (default / --advisory)
//   · editor hooks, if wired (--hook-stop / --hook-posttooluse)
//
// Flags:
...
const CONTRAST_CHECK = join(REPO_ROOT, "scripts/contrast/contrast-check.mjs");
const CONTRAST_COVERAGE = join(REPO_ROOT, "scripts/contrast/contrast-coverage.mjs");
...
function gather(ctx) {
  const results = [];
  // 1. Config integrity used to validate the Claude workflow files; that
  //    directory no longer ships with the site, so the gate is retired.
  results.push(result("config-integrity", ctx.config.severity["config-integrity"], [], { skipped: true, note: "retired: no .claude workflow config in this repository" }));
...
if (has("--hook-posttooluse")) {
  ...
  // Audit build OUTPUT only — never tooling, repo meta docs
